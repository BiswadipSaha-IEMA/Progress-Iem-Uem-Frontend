...
export default function FacultyFDPComp() {
  ...
  useEffect(() => {
    ...
    getBPData();
  }, [showPopUp]); // Re-run on showPopUp change
  ...
  const handleSearch = (event) => {
    ...
    const filteredData = data1.filter(item =>
      ...
      item.collegeName.toLowerCase().includes(searchData)
    );
  ...
  const columnHeaders = ['Organized By', 'College Name', 'Topic Name', 'Attended By', 'Date', 'Status', 'Proof Of Document', 'Created By', 'Created By Email'];
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <div className="flex-1 overflow-auto px-4 sm:px-10">
                      ...
                      <tbody>
                        {currentRows.map((item, rowIndex) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-2 whitespace-nowrap">{item.collegeName}</td>
        ...
        {/* Add FDP Pop-up */}
        {showPopUp && (
          <FDPPopUp setUtilFor={"bpAddForm"} setShowPopup={setShowPopUp} />
        )}
