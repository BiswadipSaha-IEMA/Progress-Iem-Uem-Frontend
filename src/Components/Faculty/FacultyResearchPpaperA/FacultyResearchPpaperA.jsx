           ...
           try {
            ...
            if(response.success){
                ...
                response.data.data.map(
                    (data)=>{
                       if( data.publicationGrade==="Grade-A")
                        arr.push(data)
                    }
                )
                ...
                //     }
                // )
                // console.log('kcgbdhbwouijhuqbciueqhncin',filteredData)
                setData(arr)
                setData1(arr)
            } 
    ...
    <div className="w-full p-4 md:p-8 pt-0">
        <div className={`shadow-2xl rounded-md p-4 md:p-8 ${BpNumber <= 8 ? 'h-[600px] md:h-[800px]' : 'h-auto'}`}>
            ...
            <div className="flex flex-wrap gap-4 md:gap-8 w-full h-full">
                ...
                {
                    data?.map((item, index) => (
                        <FacultyResearchPpaperACard key={index} status={item.status} title={item.title} date={item.date} name={item.name} ISBN={item.isbn} driveFileUrl={item.proofDocument||''}/>
                    ))
                }
...
export default FacultyResearchPpaperA