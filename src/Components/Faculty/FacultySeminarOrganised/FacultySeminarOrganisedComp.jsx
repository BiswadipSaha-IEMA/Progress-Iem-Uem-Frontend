...
import FacultySeminarOrganizedCard from './FacultySeminarOrganisedCard'
// import FacultySeminarOrganizedCard from './FacultySeminarOrganizedCard'

function FacultySeminarOrganizedComp() {
    const BpNumber=30
    ...
    useEffect(()=>{
        const getBPData= async()=>{
           try {
            const response= await getReq('api/v1/document/getAllEvents',
                accessToken
            )
            if(response.success){
    ...
    <div className="w-full p-4 md:p-8 pt-0">
        <div className={`shadow-2xl rounded-md p-4 md:p-8 ${BpNumber <= 8 ? 'h-[600px] md:h-[800px]' : 'h-auto'}`}>
            ...
            <div className="flex flex-wrap gap-4 md:gap-8 w-full h-full">
                ...
                {
    data
        ?.filter(item => item.eventType === 'Seminar') 
        .map((item, index) => (
            <FacultySeminarOrganizedCard
                key={index}
                status={item.status}
...
export default FacultySeminarOrganizedComp