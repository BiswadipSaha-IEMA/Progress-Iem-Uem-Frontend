...
import FacultyTriMentoringCard from './FacultyTriMentoringCard'

function FacultyTriMentoring() {
    const BpNumber=30
  ...
  return (
    ...
    <div className="relative flex flex-col md:flex-row gap-4 md:gap-8 p-4 md:p-8">
        ...
        <div className="relative w-full">
            <input
                type="text"
                placeholder="search for a Moocs"
    ...
    <div className="w-full p-4 md:p-8 pt-0">
        <div className={`shadow-2xl rounded-md p-4 md:p-8 ${BpNumber <= 8 ? 'h-[600px] md:h-[800px]' : 'h-auto'}`}>
            ...
            <div className="flex flex-wrap gap-4 md:gap-8 w-full h-full">
                ...
                {
     data
     ?.filter(item => item.eventType === 'Tri-Mentoring') // Filter only 'Workshop' type events
     .map((item, index) => (
         <FacultyTriMentoringCard
             key={index}
             status={item.status}
             title={item.organizedBy}
             date={item.date}
...
export default FacultyTriMentoring