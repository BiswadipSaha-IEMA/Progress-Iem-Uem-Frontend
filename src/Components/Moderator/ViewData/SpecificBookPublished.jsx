...
function BookPublished() {
  ...
  const navTo=sessionStorage.getItem('nav')

  const getId= sessionStorage.getItem('id')

  console.log('uhnauion----------------------------------------------------------------------',getId)
...
// Industrial Tour
  ...
  // Fetch all publication information on component mount
  useEffect(() => {
    const allInfo = async () => {
      const response = await getReq(
        `api/v1/document/getAllPublications?userId=${getId}`,
        accessToken
      );
      if (response.success) {
        setStoreData(response.data.data);
        setBookDataSubmittedArrState(response.data.userSubmittedCounts);
        console.log('Fetched Publications:', response);
      }
    };
    allInfo();
  }, []);
    ...
    const ConferenceDataFunc = async () => {
      try {
        const seminarData = await getReq(
          `api/v1/document/getAllEvents?userId=${getId}`,
          accessToken
        );
        console.log("Conference data:", seminarData);

        let arr = [];

        if (seminarData) {
          // Filter items based on event type "Conference"
          seminarData.data.data.forEach((item) => {
            if (item.eventType === "Conference") arr.push(item);
          });
          setConfArrState(arr);
          setConfOrg(arr);
        }
  ...
  // Log conference data state when it changes
  useEffect(() => {
    if (confOrg) console.log("Conference Data State:", confOrg);
  }, [confOrg]);
  ...
  useEffect(() => {
    const patentDataFunc = async () => {
      try {
        const seminarData = await getReq(
          `api/v1/document/getAllPatents?userId=${getId}`,
          accessToken
        );
        console.log("Patent data:", seminarData);

        if (seminarData) {
          setPatentOrg(seminarData.data.data);
          setPatentArrState(seminarData.data.data);
          console.log("Fetched Patents:", seminarData.data.data);
        }
      } catch (error) {
  ...
  // Log patent data state when it changes
  useEffect(() => {
    if (patentOrg) console.log("Patent Data State:", patentOrg);
  }, [patentOrg]);
  ...
  useEffect(() => {
    const getMOOCData = async () => {
      try {
        const response = await getReq(`api/v1/document/getAllMoocs?userId=${getId}`, accessToken);
        if (response.success) {
  ...
  useEffect(() => {
    const getTriMentoringData = async () => {
      try {
        const response = await getReq(`api/v1/document/getAllEvents?userId=${getId}`, accessToken);
        if (response.success) {
          console.log(response.data);
    ...
    const studentChapterFunc = async () => {
      try {
        const studentChapterData = await getReq(
          `api/v1/document/getAllStudentChapters?userId=${getId}`,
          accessToken
        );
        ...
        <div className="mt-10 h-[150px] overflow-hidden custom-scrollbar rounded-[10px]">
          {recordsOfBp.length === 0 ? (
            ...
            <DataTable
              ...
              defaultSortAsc={true}
              pagination
              paginationRowsPerPageOptions={[10, 25, 50]} // Allows user to select rows per page
